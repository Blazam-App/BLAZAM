    ...
    public class ADUser : GroupableDirectoryAdapter, IADUser
    {
        ...
        public bool SetPassword(SecureString password, bool requireChange = false)
        {
            ...
            try
            {
                ...
                //this.SetCustomProperty("password", password);
                //CommitChanges();
                if (this.Invoke("SetPassword", new object[] { password.ToPlainText() }))
                {
                    return true;
                }
          else
            ...
            using (PrincipalContext pContext = new PrincipalContext(
                    ...
                    name: DirectorySettings.ServerAddress + ":" + DirectorySettings.ServerPort,
                    container: DirectorySettings.ApplicationBaseDN,

                    userName: DirectorySettings.Username,
                    password: Encryption.Instance.DecryptObject<string>(DirectorySettings.Password)))
            {
